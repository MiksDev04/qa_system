...
// Segmentation
$segWhere = ['1=1']; $segParams = []; $segTypes = '';
...
LEFT JOIN (
    ...
    WHERE sq.question_type = 'rating' AND sa.rating IS NOT NULL AND sa.rating BETWEEN 1 AND 5
...
$role_breakdown = [];
while($seg = $seg_result->fetch_assoc()) $role_breakdown[] = $seg;

// Avg rating stat
$avg_where = ['1=1']; $avg_params = []; $avg_types = '';
if($survey_filter){ $avg_where[] = 'sq.survey_id = ?'; $avg_params[] = $survey_filter; $avg_types .= 'i'; }
if($role_filter)  { $avg_where[] = 'sr.respondent_role = ?'; $avg_params[] = $role_filter; $avg_types .= 's'; }
$avg_sql = "SELECT AVG(response_avg) as avg FROM (
    ...
    WHERE " . implode(' AND ', $avg_where) . "
      AND sq.question_type = 'rating' AND sa.rating IS NOT NULL AND sa.rating BETWEEN 1 AND 5
    GROUP BY sa.response_id
) t";
$avg_stmt = $conn->prepare($avg_sql);
if($avg_types) $avg_stmt->bind_param($avg_types,...$avg_params);
$avg_stmt->execute();
$avg_q = $avg_stmt->get_result()->fetch_assoc()['avg'];

// Today count
$today_where = ['DATE(sr.submitted_at) = CURDATE()']; $today_params = []; $today_types = '';
if($survey_filter){ $today_where[] = 'sr.survey_id = ?'; $today_params[] = $survey_filter; $today_types .= 'i'; }
if($role_filter)  { $today_where[] = 'sr.respondent_role = ?'; $today_params[] = $role_filter; $today_types .= 's'; }
$today_sql = "SELECT COUNT(*) as c FROM survey_responses sr WHERE " . implode(' AND ', $today_where);
$today_stmt = $conn->prepare($today_sql);
if($today_types) $today_stmt->bind_param($today_types,...$today_params);
$today_stmt->execute();
...
if($is_partial){
    ...
    <?php else: foreach($role_breakdown as $seg):
        $segCount = (int)$seg['response_count'];
        $share = $total > 0 ? round(($segCount / $total) * 100, 1) : 0; ?>
        <tr>
            ...
            <td>
                ...
                <?php else: ?><span class="text-muted-qa">N/A</span><?php endif; ?>
            </td>
        </tr>
    <?php endforeach; endif;
    $seg_html = ob_get_clean();

    // Response table body
    ob_start();
    if($responses->num_rows === 0): ?>
        <tr><td colspan="8"><div class="empty-state"><i class="bi bi-inbox"></i><p>No responses found</p></div></td></tr>
    <?php else: $n=$offset+1; while($res=$responses->fetch_assoc()): ?>
        <tr>
            ...
            <td>
                ...
                <span class="fw-600" style="color:var(--warning)"><i class="bi bi-star-fill"></i> <?= number_format($res['avg_rating'],1) ?></span>
                <?php else: echo '<span class="text-muted-qa">N/A</span>'; endif; ?>
            </td>
            ...
            <td>
                <button class="btn-qa btn-qa-secondary btn-qa-sm btn-qa-icon" onclick="viewResponse(<?= $res['response_id'] ?>)" title="View answers">
                    <i class="bi bi-eye"></i>
                </button>
            </td>
        </tr>
...
<div class="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2">
    <span class="text-muted-qa" id="responseCount">Showing <?= min($offset+1,$total) ?>-<?= min($offset+$per_page,$total) ?> of <?= $total ?> responses</span>
...
<?php
$_page_js       = (int)$page;
$_total_pages_js = (int)$total_pages;
$_survey_js     = json_encode((string)$survey_filter);
$_role_js       = json_encode($role_filter);

$extra_js = <<<ENDJS
<script>
var currentPage     = {$_page_js};
var totalPages      = {$_total_pages_js};
var currentSurvey   = {$_survey_js};
var currentRole     = {$_role_js};

$(function(){
    buildPagination("paginationContainer", currentPage, totalPages, "goPage");
});

function loadResponses(page, survey, role){
    $.get("responses.php", {partial:1, page:page, survey_id:survey, role:role}, function(res){
        currentPage   = res.page;
        totalPages    = res.total_pages;
        currentSurvey = survey;
        currentRole   = role;

        $("#statsContainer").html(res.stats);
        $("#segBody").html(res.seg);
        $("#responsesBody").html(res.rows);

        var from = res.total === 0 ? 0 : res.offset + 1;
        var to   = Math.min(res.offset + res.per_page, res.total);
        $("#responseCount").text("Showing " + from + "-" + to + " of " + res.total + " responses");
...
function goPage(p){
    loadResponses(p, currentSurvey, currentRole);
}

function applyFilters(){
    loadResponses(1, $("#f-survey").val(), $("#f-role").val());
}
...
function formatSubmittedAt(value){
    ...
    if(isNaN(parsed.getTime())) return escapeHtml(raw);
    return escapeHtml(parsed.toLocaleString("en-US", {year:"numeric",month:"short",day:"2-digit",hour:"2-digit",minute:"2-digit"}));
}

function renderRating(ratingValue){
    var rating = Math.max(0, Math.min(5, Number(ratingValue) || 0));
    if(!rating) return '<span class="text-muted-qa">No rating provided</span>';
    var stars = "";
    for(var i = 0; i < 5; i++){
...
function renderAnswer(answer){
    var questionType = String(answer.question_type || "").toLowerCase();
    if(questionType === "rating") return renderRating(answer.rating);
    var answerText = String(answer.answer_text ?? "").trim();
    if(!answerText) return '<span class="text-muted-qa">No answer provided</span>';
...
function viewResponse(responseId){
    ...
    $.get("/qa_system/api/responses.php", {action:"get_detail", response_id:responseId}, function(res){
        ...
        var d = res.data || {};
        var html = '<div class="mb-3 p-3" style="background:var(--bg-hover);border-radius:var(--radius-sm)">' +
            '<div class="row g-2">' +
        ...
        if(Array.isArray(d.answers) && d.answers.length > 0){
            $.each(d.answers, function(i, a){
                var questionText = textOrFallback(a.question_text, "Question " + (i + 1));
